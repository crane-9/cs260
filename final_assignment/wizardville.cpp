...
// Anonymous namespace, just to keep this secret to this file. All helpers in filling the one func of story.h
namespace {
    // Callbacks

    string tutDoubt(StoryNode *n, GraphMap *m, Player *p) { p->addFlag("tutorialDoubt"); return ""; }

    string tutAssurance(StoryNode *n, GraphMap *m, Player *p) {
    ...
    void createTutorial(GraphMap *graph) {
    ...
    void createGame(GraphMap *graph) {
        graph->addVertex(new StoryNode(
